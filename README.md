...
Once the .yaml file is started, open a web browser and enter `https://[IP_Address_Provided_By_The_Script]:443` (the PHP and Nginx container provides the user interface). It's not possible to connect with the HTTP protocol, it's automatically redirected to HTTPS. The IP address is the Haproxy container which will distribute the load between two nginx WEB containers. Make sure to add the port. 
...
### Installation of syslog (Optional)

If you want to view the application logs as wall, that's possible (optionally). The logs (a part) of the application's containers are managed by podman and stored in the host machine's journalctl. You need to configure two things. The first is that the syslog container listens on a port to receive information (logs), which is already done. The second is to manually send the logs from the containers to a specific port. We haven't been able to automate this task, si it needs to be done manually. 
1) First, find the journald.conf file based on your OS (use the command `find /etc -name "journald.conf" -print`. It's often located in /etc/systemd/journald.conf. In this file, uncomment the line `ForwardToSyslog=Yes`. And restart the journalctl service with : `systemctl|service restart systemd-journald`.
2) As mentioned earlier, make sure to add the line `unqualified-search-registries = ['docker.io']` in the file /etc/containers/registries.conf and retrieve the Balabit syslog image using the following command : `podman pull balabit/syslog-ng:latest` and restart the container(s) : syslog-ng or syslog-ng and all others.
3) Then, run the rsyslog.sh script (`bash rsyslog.sh`|`./rsyslog.sh`). This part will enable the fact of senting logs to the syslog-ng's container. 
4) Finally, to be sure, restart the service (`systemctl|service restart rsyslog`).
5) To view the application logs, connect to the syslog-ng container (`podman exec -it syslog-ng /bin/bash`) and navigate to `/var/log/YLN`. Files like nginx1.log, php.log should be created when using the application (for example when navigating between pages).
...
(AND Possibly:) - docker.io/balabit/syslog-ng                   latest      
...
--> L'adresse IP de l'application sur laquelle se rendre est https://172.18.0.7:443
